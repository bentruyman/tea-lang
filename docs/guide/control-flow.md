...
while index < @len(names) && !found
  if names[index] == search_name
    found = true
    @println(`Found ${search_name} at position ${index}`)
  end
  index = index + 1
end

if !found
  @println(`${search_name} not found`)
end
```
