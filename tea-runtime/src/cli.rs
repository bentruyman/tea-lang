...
pub struct CliParseOutcome {
    ...
    pub options: HashMap<String, RuntimeValue>,
    pub positionals: HashMap<String, RuntimeValue>,
    pub rest: Vec<String>,
    pub message: String,
    pub help: String,
}
...
fn parse_command_spec(value: &RuntimeValue, program_name: Option<&str>) -> Result<CommandSpec> {
...
fn parse_option_spec(value: &RuntimeValue) -> Result<OptionSpec> {
    ...
    let default = if let Some(default_value) = map.get("default") {
        if matches!(default_value, RuntimeValue::Nil) {
            OptionDefault::None
        } else {
            match kind {
                OptionKind::Flag => match default_value {
                    RuntimeValue::Bool(flag) => OptionDefault::Bool(*flag),
                    _ => bail!(format!("flag option '{}' default must be Bool", name)),
                },
                OptionKind::Value => {
                    if multiple {
                        match default_value {
                            RuntimeValue::List(items) => OptionDefault::Many(items.clone()),
                            RuntimeValue::Nil => OptionDefault::Many(Vec::new()),
                            _ => bail!(format!(
...
fn parse_positional_spec(value: &RuntimeValue) -> Result<PositionalSpec> {
...
fn initialise_option_state<'a>(spec: &'a CommandSpec) -> Result<Vec<OptionParseState<'a>>> {
    spec.options
        .iter()
        .map(|option| {
...
fn handle_option(
    ...
    match (&state.spec.kind, &mut state.value) {
        ...
        (OptionKind::Value, OptionValueState::Single { value, provided }) => {
            ...
            let coerced = coerce_value(&RuntimeValue::String(raw.clone()), &state.spec.value_type)
...
fn collect_option_values(states: &[OptionParseState]) -> HashMap<String, RuntimeValue> {
    let mut map = HashMap::new();
    for state in states {
        let value = match &state.value {
            OptionValueState::Flag { value } => RuntimeValue::Bool(*value),
            OptionValueState::Single { value, .. } => value.clone().unwrap_or(RuntimeValue::Nil),
            OptionValueState::Multi { values, .. } => RuntimeValue::List(values.clone()),
        };
...
fn parse_positionals(
    ...
    args: &[String],
) -> Result<(HashMap<String, RuntimeValue>, usize), String> {
    let mut consumed = 0usize;
    ...
    for positional in specs {
        if positional.multiple {
            ...
            result.insert(positional.name.clone(), RuntimeValue::List(values));
            ...
            result.insert(positional.name.clone(), RuntimeValue::Nil);
        }
...
fn current_scope(
    ...
    positionals: HashMap<String, RuntimeValue>,
    ...
    if !spec.options.is_empty() {
        ...
        for option in &spec.options {
            ...
            if option.required {
                ...
                entry.push_str(&format!(" (default: {})", display_value(value)));
...
fn display_value(value: &RuntimeValue) -> String {
    match value {
        ...
        RuntimeValue::Dict(_) => "<dict>".to_string(),
    }
}
...
fn get_string(map: &HashMap<String, RuntimeValue>, key: &str) -> Result<String> {
    match map.get(key) {
        Some(RuntimeValue::String(value)) => Ok(value.clone()),
        Some(RuntimeValue::Nil) | None => Ok(String::new()),
        _ => bail!("field '{}' must be a String", key),
    }
}

fn get_optional_string(map: &HashMap<String, RuntimeValue>, key: &str) -> Result<Option<String>> {
    match map.get(key) {
        Some(RuntimeValue::String(value)) => Ok(Some(value.clone())),
        Some(RuntimeValue::Nil) | None => Ok(None),
        _ => bail!("field '{}' must be a String", key),
    }
}

fn get_bool(map: &HashMap<String, RuntimeValue>, key: &str, default: bool) -> Result<bool> {
    match map.get(key) {
        Some(RuntimeValue::Bool(value)) => Ok(*value),
        Some(RuntimeValue::Nil) | None => Ok(default),
        _ => bail!("field '{}' must be a Bool", key),
    }
}

fn get_string_list(map: &HashMap<String, RuntimeValue>, key: &str) -> Result<Vec<String>> {
    match map.get(key) {
        Some(RuntimeValue::List(items)) => {
            let mut result = Vec::with_capacity(items.len());
            for item in items.iter() {
                match item {
                    RuntimeValue::String(value) => result.push(value.clone()),
                    _ => bail!("field '{}' must be a list of Strings", key),
                }
            }
            Ok(result)
        }
        Some(RuntimeValue::Nil) | None => Ok(Vec::new()),
        _ => bail!("field '{}' must be a List", key),
    }
}

fn parse_value_type(map: &HashMap<String, RuntimeValue>, key: &str) -> Result<ValueType> {
    match map.get(key) {
        Some(RuntimeValue::String(value)) => match value.to_ascii_lowercase().as_str() {
        ...
        Some(RuntimeValue::Nil) | None => Ok(ValueType::String),
...
fn expect_dict<'a>(
    ...
    context: &str,
) -> Result<&'a HashMap<String, RuntimeValue>> {
    match value {
        RuntimeValue::Dict(entries) => Ok(entries),
        _ => bail!("expected {} to be a dictionary", context),
    }
}

fn expect_list(value: &RuntimeValue, context: &str) -> Result<Vec<RuntimeValue>> {
    match value {
...
fn coerce_value(value: &RuntimeValue, ty: &ValueType) -> Result<RuntimeValue> {
    match (ty, value) {
        (ValueType::String, RuntimeValue::String(_)) => Ok(value.clone()),
        (ValueType::String, other) => Ok(RuntimeValue::String(display_value(other))),
        (ValueType::Int, RuntimeValue::Int(_))
        | (ValueType::Float, RuntimeValue::Float(_))
        | (ValueType::Bool, RuntimeValue::Bool(_)) => Ok(value.clone()),
        (ValueType::Int, RuntimeValue::String(text)) => {
            let parsed: i64 = text
                .parse()
                .map_err(|err| anyhow!("failed to parse int: {}", err))?;
            Ok(RuntimeValue::Int(parsed))
        }
        (ValueType::Float, RuntimeValue::String(text)) => {
            let parsed: f64 = text
                .parse()
                .map_err(|err| anyhow!("failed to parse float: {}", err))?;
            Ok(RuntimeValue::Float(parsed))
        }
        (ValueType::Bool, RuntimeValue::String(text)) => {
            let lower = text.to_ascii_lowercase();
            match lower.as_str() {
                "true" | "1" | "yes" | "y" => Ok(RuntimeValue::Bool(true)),
                "false" | "0" | "no" | "n" => Ok(RuntimeValue::Bool(false)),
                _ => bail!("failed to parse bool"),
            }
        }
        (ValueType::Int, RuntimeValue::Float(value)) => Ok(RuntimeValue::Int(*value as i64)),
        (ValueType::Float, RuntimeValue::Int(value)) => Ok(RuntimeValue::Float(*value as f64)),
        (_, RuntimeValue::Nil) => Ok(RuntimeValue::Nil),
        _ => bail!("unsupported default value type"),
    }
}

fn coerce_string(raw: String, ty: &ValueType) -> Result<RuntimeValue, String> {
    match ty {
        ValueType::String => Ok(RuntimeValue::String(raw)),
        ValueType::Int => raw
            .parse::<i64>()
