<ContentPage
  eyebrow="Language"
  title="Modules"
  description='Tea uses `use alias = "module.path"` imports for stdlib modules and relative Tea files.'
>
  ...
  <NoteCard title="Stdlib imports">
    Checked-in examples use imports like <code>use args = "std.args"</code>, <code>use fs = "std.fs"</code>, and <code>use regex = "std.regex"</code>. Each stdlib module exposes its public API from a <code>mod.tea</code> entrypoint.
  </NoteCard>
