...
const manifestEntries = new Map(
  referenceManifest.entries.map((entry) => [entry.slug, entry]),
);

function resolveReferenceItem(item: {
  slug: string;
  href: string;
}): ReferenceItem {
...
const referenceItemMap = new Map(
  referenceItems.map((item) => [item.slug, item]),
);
