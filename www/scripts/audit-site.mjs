...
function gatherStaticRoutes() {
  const pageFiles = walk(path.join(appRoot, "app")).filter((file) =>
    /page\.(tsx|mdx)$/.test(file),
  );
  return pageFiles
...
function collectHrefs() {
  const files = walk(path.join(appRoot, "app")).concat(
...
function loadReferenceManifest() {
  assert(
    ...
    "Missing generated reference manifest: generated/reference-manifest.json",
  );
...
function verifyReferenceSources() {
  ...
  assert(
    ...
    "Missing dynamic reference route: app/reference/[slug]/page.tsx",
  );
  ...
  const configuredSlugs = new Set(
    siteMap.referenceSections.flatMap((section) =>
  ...
  for (const slug of configuredSlugs) {
    assert(
      manifestEntries.has(slug),
      `Missing reference manifest entry for slug: ${slug}`,
    );
  }

  for (const slug of manifestEntries.keys()) {
    assert(
      configuredSlugs.has(slug),
      `Reference manifest entry is not present in site-map.json: ${slug}`,
    );
  }
  ...
  for (const entry of manifest.entries) {
    ...
    assert(
      entry.summary,
      `Reference manifest entry has no summary: ${entry.slug}`,
    );
    assert(
      Array.isArray(entry.functions),
      `Reference manifest entry has invalid functions: ${entry.slug}`,
    );
    assert(
      entry.functions.length > 0,
      `Reference manifest entry has no functions: ${entry.slug}`,
    );
    for (const fn of entry.functions) {
      assert(fn.name, `Reference function is missing a name in ${entry.slug}`);
      assert(
        fn.signature_display,
        `Reference function is missing a signature in ${entry.slug}:${fn.name}`,
      );
      assert(
        fn.summary,
        `Reference function has no summary in ${entry.slug}:${fn.name}`,
      );
    }
  }
  ...
  for (const section of siteMap.exampleSections) {
    for (const item of section.items) {
      ...
      assert(
        ...
        `Missing example source: ${item.sourcePath}`,
      );
      if (item.readmePath) {
        assert(
          fs.existsSync(path.join(repoRoot, item.readmePath)),
          `Missing example README: ${item.readmePath}`,
        );
      }
    }
  }
...
function verifyBannedPatterns() {
  ...
  assert(
    hits.length === 0,
    `Banned stale snippet patterns found:\n${hits.join("\n")}`,
  );
