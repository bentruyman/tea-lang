...
function exitForMissingTool(lines) {
  if (optional) {
    ...
    console.warn(
      "Continuing without Tea WASM assets because this run is optional.",
    );
    process.exit(0);
  }
...
if (optional && result.status && result.status !== 0) {
  console.warn(
    "Tea WASM bundle build failed during optional setup. Continuing anyway.",
  );
  process.exit(0);
}
